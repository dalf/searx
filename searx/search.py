...
async def search_one_http_request(engine, query, request_params):
...
async def search_one_offline_request(engine, query, request_params):
    # Note: engine.search function doesn't have a loop
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, engine.search, query, request_params)
...
async def search_one_request_safe(engine_name, query, request_params, result_container, start_time, timeout_limit):
    if engines[engine_name].offline:
        return await search_one_offline_request_safe(engine_name, query, request_params, result_container, start_time, timeout_limit)  # noqa
    return await search_one_http_request_safe(engine_name, query, request_params, result_container, start_time, timeout_limit)


async def search_one_offline_request_safe(engine_name, query, request_params, result_container, start_time, timeout_limit):
    engine = engines[engine_name]

    try:
        search_results = await search_one_offline_request(engine, query, request_params)
...
async def search_one_http_request_safe(engine_name, query, request_params, result_container, start_time, timeout_limit):
    ...
    try:
        ...
        search_results = await search_one_http_request(engine, query, request_params)
